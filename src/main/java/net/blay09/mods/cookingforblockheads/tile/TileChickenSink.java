...
public class TileChickenSink extends TileEntity implements ITickable {

    private static final int HATCH_TIME = 20 * 60;
    private static final int PLOP_TIME = 20 * 60 * 2;
    ...
    private final DoorAnimator doorAnimator = new DoorAnimator(this, 1, 2).setOpenRadius(2);

    private boolean isHatched;
    private int hatchTime;
    private int eggPlopTime;
    private boolean isDirty;

    public boolean startHatching() {
        if(!isHatched) {
    ...
    public void update() {
        ...
        if(isHatched) {
            eggPlopTime++;
            if(eggPlopTime >= PLOP_TIME) {
                // TODO plop
                ItemHandlerHelper.insertItem(itemHandler, new ItemStack(Items.EGG), false);
                eggPlopTime = 0;
            }
        } else if(hatchTime > 0) {
            hatchTime--;
    ...
    public void readFromNBT(NBTTagCompound tagCompound) {
        ...
        isHatched = tagCompound.getBoolean("IsHatched");
        hatchTime = tagCompound.getInteger("HatchTime");
        eggPlopTime = tagCompound.getInteger("EggPlopTime");
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound tagCompound) {
        ...
        tagCompound.setBoolean("IsHatched", isHatched);
        tagCompound.setInteger("EggHatchTime", hatchTime);
        tagCompound.setInteger("EggPlopTime", eggPlopTime);
